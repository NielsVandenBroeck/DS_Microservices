...
class CalendarInfo(Resource):
    def get(self):
        ...
        if not viewer_name or not calendar_user_name:
            return {'message': 'Missing arguments. viewer_name and calendar_user_name is needed.', 'success': False}, 400
        owner_id = getId(calendar_user_name)
        # Check if the owner with username exists and is present in the database
        if owner_id is None:
            return {'message': 'Given owner to watch calendar for does not exist in the database.', 'success': False}, 200
        curs = conn.cursor()
        ...
        if viewer_name != calendar_user_name:
            ...
            if viewer_id is None:
                return {'message': 'Given viewer to watch calendar does not exist in the database.', 'success': False}, 200
            # Check if the viewer had permission to watch calendar
            curs.execute("SELECT 1 FROM calendar_sharing WHERE calendar_id = %s AND viewer_id = %s;", (calendar_id, viewer_id))
        ...
        result = requests.get("http://events:5000/events/calendar", json={'event_ids': event_ids, 'user_id':owner_id}).json()
        if result['success']:
...
class AddEvent(Resource):
    def post(self):
        ...
        curs.execute("INSERT INTO calendar_events (event_id, calendar_id) VALUES (%s,(SELECT id FROM calendars WHERE user_id = %s));", (event_id, user_id))
        conn.commit()
        return {'success': True}, 200
...
class ShareCalendar(Resource):
    def post(self):
        ...
        if owner_id is None:
            return {'message': 'Given owner to share the calendar does not exist in the database.', 'success': False}, 400
        ...
        curs.execute("SELECT 1 FROM calendar_sharing WHERE viewer_id = %s and calendar_id = (SELECT id FROM calendars WHERE user_id = %s);", (viewer_id, owner_id))
        ...
        curs.execute("INSERT INTO calendar_sharing (viewer_id, calendar_id) VALUES (%s,(SELECT id FROM calendars WHERE user_id = %s));",(viewer_id, owner_id))
        conn.commit()
        return {'success': True}, 200
