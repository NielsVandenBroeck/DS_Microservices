...
class CreateEvent(Resource):
    def post(self):
        ...
        title, description, date, is_public, organizer = request_data.get('title'), request_data.get('description'), request_data.get('date'), True if request_data.get(
        ...
        if not title or not date or not organizer or is_public is None:
            return {'message': 'Missing arguments. title, date, organizer and publicity is needed.', 'success': False}, 400
        ...
        curs.execute("INSERT INTO events (event_date, organizer_id, title, description, is_public) VALUES (%s, %s, %s, %s, %s) RETURNING id;", (date, organizer_id, title, description, is_public))
...
class ListInvites(Resource):
    def get(self):
        ...
        curs.execute("SELECT events.id, events.title, events.event_date, events.organizer_id, events.is_public FROM events INNER JOIN event_invitations ON events.id = event_invitations.event_id WHERE event_invitations.user_id = %s;",(user_id,))
        result = []
        for row in curs.fetchall():
            # For each event, convert the date to readable text and get the username of the organizer
...
class AnswerInvite(Resource):
    def post(self):
        request_data = request.json
        username, event_id, participation = request_data.get('username'),request_data.get('event_id'),request_data.get('participation').replace("'","")
        # username, event_id, participation should be given
        if not username or event_id is None or not participation:
            return {'message': 'Missing arguments. the username, event_id and participation is needed.', 'success': False}, 400
        user_id = getId(username)
        # Check if the user with username exists and is present in the database
        if user_id is None:
        ...
        curs.execute("INSERT INTO event_participation (event_id, user_id, response) VALUES (%s, %s, %s);",(event_id, user_id, participation))
...
class ListCalendarEvents(Resource):
    def get(self):
        ...
        for event_id in event_ids:
            ...
            response = curs.fetchone()
            organizer = getUsername(response[2])
            curs.execute("SELECT response FROM event_participation WHERE event_id = %s and user_id = %s;", (event_id, user_id))
            participation = curs.fetchone()[0]
            result.append((event_id, response[0],response[1].strftime('%d %B %Y'),organizer,participation,'Public' if response[3] else 'Private'))
        return {'events': result, 'success': True}, 200


api.add_resource(ListPublicEvents, '/events/public')
api.add_resource(CreateEvent, '/events/create')
api.add_resource(InviteUser, '/events/invite')
api.add_resource(ListInvites, '/events/invites')
api.add_resource(AnswerInvite, "/events/participation")
api.add_resource(EventInfo, '/events/info')
api.add_resource(ListCalendarEvents, '/events/calendar')