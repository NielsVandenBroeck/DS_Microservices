...
def home():
    ...
    if username is None:
        ...
        # =================================
        response = requests.get("http://events:5000/events/public").json()
...
def create_event():
    ...
    #==========================
    global username
    response = requests.post("http://events:5000/events/create",
                  json={'title': title, 'description': description,'date':date, 'publicprivate': publicprivate,
    ...
    for user in invites.replace(" ", "").split(';'):
        requests.post("http://events:5000/events/invite", json={'event_id':event_id, 'user':user})
    return redirect('/')
...
def calendar():
    ...
    response = requests.get("http://calendars:5000/calendar", json={'username': username, 'calendar_user': calendar_user}).json()
    success = response['success']
    if success:
...
def share():
    ...
    response = requests.post("http://calendars:5000/calendar/share", json={'calendar_user': username, 'shared_user': share_user}).json()
    success = response['success']
...
def view_event(eventid):
    ...
    response = requests.get("http://events:5000/events/info", json={'username': username, 'event_id': eventid}).json()
    success = response['success']
    if success:
...
def register():
    ...
    # ================================
    response = requests.post("http://users:5000/user/register", json={'username': req_username, 'password': req_password}).json()
    success = response['success']
    save_to_session('success', success)
...
def invites():
    ...
    #==============================
    response = requests.get("http://events:5000/events/invites", json={'username': username}).json()
    success = response['success']
    if success:
...
def process_invite():
    ...
    #=======================
    requests.post("http://events:5000/events/participation", json={'username':username, 'event_id': eventId, 'participation':status})
